...
def cal_avg_at_k(
    ...
    """
    Calculates the average correctness rate across all problems.
    
    Args:
        num_samples: Integer, list, or array representing the number of samples for each problem.
        num_correct: List or array representing the number of correct samples for each problem.
    
    Returns:
        The average correctness rate as a float.
    """
    if isinstance(num_samples, int):
        total_samples = num_samples * len(num_correct)
    else:
        if len(num_samples) != len(num_correct):
            raise ValueError("Length of num_samples must match length of num_correct")
...
def evaluate(data_name, prompt_type, samples: list=None, file_path: str=None, max_num_samples=None, execute=False):
    ...
    avg_at_k = cal_avg_at_k(max_len, num_correct)
    ...
    result_json = {
        ...
        "pass@k": pass_at_k,
        "avg@k": np.round(avg_at_k*100, decimals=1)
    }
