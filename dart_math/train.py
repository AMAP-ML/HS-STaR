...
def tokenize_fn(
    ...
    tokenized_dict = tokenizer(
        ...
        truncation=True,
        add_special_tokens=False # important for deepseekmath
...
class TokenizedSupervisedDataset(Dataset):
    ...
    def load_from_raw_dset(
        ...
        TokenizedSupervisedDataset
        """
        logging.info(f"Loading from raw dataset {data_path}...")
        try:
            dset = load_dataset(data_path)["train"]
        except:
            dset = load_dataset(data_path, features=Features({
                "dataset": Value("string"),
                "ref_ans": Value("string"),
                "ans": Value("string"),
                "correct": Value("bool"),
                "abs_tol": Value("null"),          # 全部为 null
                "finish_reason": Value("string"),
                "stop_reason": Value("string"),    # 允许字符串或 null
                "cumulative_logprob": Value("null"),
                "n_trials": Value("int64"),        # 注意实际类型是 int64，不是 int32
                "n_corrects": Value("int64"),
                "n_shots": Value("int64"),
                "max_n_trials": Value("int64"),
                "min_n_corrects": Value("int64"),
                "agent": Value("string"),
                "prompt_template": Value("string"),
                "query": Value("string"),
                "resp": Value("string"),
            }))["train"]
        logging.info(f"Raw dataset len = {len(dset)}")
        ...
        # if 'Qwen2.5' in tokenizer.name_or_path and 'Math' in tokenizer.name_or_path:
        #     targets = [f"{resp}<|im_end|>" for resp in resps]
        # else:
    ...
    def pad(self) -> None:
        ...
        for i in range(len(self.input_ids)):
            ...
            if pad_len != 0:
                ...
                self.input_ids[i] = self.input_ids[i].to(torch.int64)
                self.labels[i] = self.labels[i].to(torch.int64)
                self.attention_mask[i] = self.attention_mask[i].to(torch.int64)
...
def make_dpo_dset(
    ...
    dset = load_dataset(data_path[0])["train"]
    logging.info(f"Raw dataset len = {len(dset)}")

    prompts = dset[prompt_field]
    logging.debug("Formatting inputs ...")
    if not isinstance(prompt_template, PromptTemplate):
        prompt_template = PromptTemplate.load_from_id_or_path(prompt_template)
    sources = [prompt_template.make_full_prompt(prompt) for prompt in prompts]    
    logging.debug(f"sources[0] = {sources[0]} ...")
    chosens = dset[chosen_field]
    rejecteds = dset[rejected_field]

    # 组装DPO格式数据
    dpo_data = []
    for source, chosen, rejected in zip(sources, chosens, rejecteds):
        dpo_data.append({
            "prompt": source,
            "chosen": chosen,
            "rejected": rejected
    ...
    train_dataset = datasets.Dataset.from_list(dpo_data)
    ...
    print(f"sources[0] = { train_dataset[0]:} ...")
    print(f"sources[1] = { train_dataset[1]:} ...")
